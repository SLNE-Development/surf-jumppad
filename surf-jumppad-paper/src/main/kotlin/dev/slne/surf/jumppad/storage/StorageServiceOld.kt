...
class StorageServiceOld {
    ...
    fun loadPads(): Int {
        ...
        files.forEach { path ->
            ...
            runCatching {
                ...
                val origin = config.getLocation("pad.data.origin")?.toJumpPadPosition()
                    ?: error("Origin missing in ${path.fileName}")
                ...
                val targetLocation = config.getLocation("pad.data.targetLocation")?.toJumpPadPosition()
    ...
    fun savePads() {
        ...
        pads.forEach { pad ->
            ...
            config["pad.data.origin"] = pad.origin.toLocation()
            config["pad.data.type"] = pad.type.name
            config["pad.data.distance"] = pad.distance
            pad.targetLocation?.toLocation()?.let { config["pad.data.targetLocation"] = it }
